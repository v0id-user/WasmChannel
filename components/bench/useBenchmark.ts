...
import { WasmPacket, PacketKind as WasmPacketKind, ReactionKind as WasmReactionKind } from "@/utils/wasm/init";
import { JsonPacket, JsonPacketKind, JsonReactionKind } from "@/utils/benchmarks/jsonPacket";
import { generateRandomPayload, generateFakeMessageData, getModeDescription } from "@/utils/benchmarks/helpers";
import type { BenchmarkResult, LogEntry, BenchmarkMode, PayloadSize, ComparisonEntry, TestDuration, AutoRunCount } from "@/types/benchmark";
...
export function useBenchmark() {
	...
	const [benchmarkMode, setBenchmarkMode] = useState<BenchmarkMode>('compressed');
	...
	const addLog = useCallback((type: LogEntry['type'], message: string, details?: LogEntry['details']) => {
		...
		const logEntry: LogEntry = { timestamp, type, message, details: lightDetails };
	...
	const runBenchmark = useCallback(async (singleRun = false) => {
		setIsRunning(true);
		// Don't reset results immediately to prevent flickering
		setProgress(0);
		if (!singleRun) clearLogs();

		addLog('info', `  Starting benchmark - ${getModeDescription(benchmarkMode)} (${testDuration}s duration)`);
		addLog('warning', benchmarkMode === 'unfair' ? 
		...
		if (!isWasmWarmedUp) {
			addLog('warning', 'WASM COLD START: First run includes JIT compilation overhead');
		} else {
		...
		addLog('info', `Test payload: ${payloadSize.toUpperCase()} random data (${testPayload.length} bytes)`, {
		...
		addLog('info', `🦀 Starting Rust WASM benchmark (${testDuration} seconds)`);
		const rustStartTime = performance.now();
		let rustOperations = 0;
		let rustErrors = 0;
		
		addLog('info', 'WASM direct calls (no wrapper overhead): LZ4 compression + CRC32 + bincode');

		let lastRustLogTime = rustStartTime;
		while (performance.now() - rustStartTime < durationMs) {
			try {
				const fakeData = generateFakeMessageData();
				const packet = new WasmPacket(WasmPacketKind.Message, fakeData.messageId, fakeData.userId, WasmReactionKind.Like, testPayload);
				...
				if (deserialized) {
					...
					if (rustOperations % 500 === 0) {
						...
						addLog('info', `🦀 Rust WASM: ${rustOperations} ops (${opsPerSec.toFixed(0)} ops/sec avg)`, {
		...
		addLog('info', `📄 Starting JSON benchmark (${testDuration} seconds) - ${
			useCompression ? 'WITH compression' : 
			useBasicMode ? 'RAW mode (no compression)' : 
		...
		while (performance.now() - jsonStartTime < durationMs) {
			try {
				...
				const serialized = packet.serialize();
				const deserialized = JsonPacket.deserialize(serialized, useCompression);
				...
				if (deserialized) {
					...
					if (jsonOperations % 500 === 0) {
						...
						addLog('info', `📄 JSON: ${jsonOperations} ops (${opsPerSec.toFixed(0)} ops/sec avg)`, {
		...
		if (benchmarkMode === 'unfair') {
			addLog('warning', '⚠️ UNFAIR COMPARISON: WASM does much more work than JSON!', {
		...
		if (rustWinner) {
			const advantage = ((rustOperations / jsonOperations - 1) * 100).toFixed(1);
			...
			const advantage = ((jsonOperations / rustOperations - 1) * 100).toFixed(1);
		...
		console.log(`WASM Status: ${isWasmWarmedUp ? 'WARMED UP' : 'COLD START'}`);
		console.log(`🦀 Rust WASM: ${rustOperations.toLocaleString()} operations`);
		...
		console.log('================================');
	}, [benchmarkMode, payloadSize, testDuration, addLog, clearLogs, isWasmWarmedUp]);
	...
	const runAutoTests = useCallback(async () => {
		...
		addLog('info', `  Starting AUTO-RUN: ${autoRunCount} tests with ${testDuration}s duration each`);
		addLog('info', `Total estimated time: ${(autoRunCount * testDuration * 2 / 60).toFixed(1)} minutes`);
		...
		for (let i = 0; i < autoRunCount; i++) {
			...
			addLog('info', `🔄 Auto-run ${i + 1}/${autoRunCount}: ${randomSize.toUpperCase()} ${randomMode.toUpperCase()}`);
		...
		addLog('success', `🎉 AUTO-RUN COMPLETE: Generated ${autoRunCount} benchmark results!`);
