				...
				<div className="flex flex-col items-center space-y-6 p-8">
					...
					<div className="flex space-x-1">
						{['wasm', 'fingerprint', 'auth'].map((step) => {
							const isActive = 
								(step === 'wasm' && ['wasm-loading', 'wasm-ready'].includes(state.step)) ||
								(step === 'fingerprint' && state.step === 'fingerprinting') ||
								(step === 'auth' && state.step === 'signing-in');
							
							const isCompleted = 
								(step === 'wasm' && ['wasm-ready', 'fingerprinting', 'signing-in', 'ready'].includes(state.step)) ||
								(step === 'fingerprint' && ['signing-in', 'ready'].includes(state.step)) ||
