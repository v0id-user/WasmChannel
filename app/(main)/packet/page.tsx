...
export default function PacketPage() {
	...
	// Form state
	const [selectedKind, setSelectedKind] = useState<PacketKind>(
		PacketKind.Message,
	);
	...
	const testDeserialization = (binaryStr: string, isPrebuilt: boolean) => {
		try {
			...
			alert(
				`${isPrebuilt ? "Pre-built" : "Custom"} packet deserialized successfully!\nKind: ${PacketKind[kind]}\nMessage: ${message}`,
			);
		} catch (error) {
	...
	const generatePacketDiagram = () => {
		...
		lines.push(
			"      KIND           PAYLOAD LEN           COMPRESSED DATA        ",
		);
		lines.push(
			"   (1-8 bytes)       (variable)           (LZ4 compressed)       ",
		);
		lines.push("");
		...
		lines.push(
			"│ KIND: PacketKind enum (Message=0, Reaction=1)                    │",
		);
		lines.push(
			"│       Serialized using bincode variable-length encoding          │",
		);
		lines.push(
			"│                                                                  │",
		);
		lines.push(
			"│ PAYLOAD LEN: Vec<u8> length prefix (bincode varint)              │",
		);
		lines.push(
			"│              Can be 1-8 bytes depending on payload size          │",
		);
		lines.push(
			"│                                                                  │",
		);
		lines.push(
			"│ COMPRESSED DATA: LZ4 frame-compressed payload data               │",
		);
		lines.push(
			"│                 Original payload is compressed before storage    │",
		);
		lines.push(
		...
		lines.push(
			"│ CRC32: 32-bit cyclic redundancy check for data integrity         │",
		);
		lines.push(
			"│        Calculated from compressed payload bytes                  │",
		);
		lines.push(
			"└──────────────────────────────────────────────────────────────────┘",
		);
		...
		lines.push(
			"COMPRESSION: LZ4 Frame Format (High compression ratio, fast decompression)",
		);
			...
			<div className="max-w-4xl mx-auto px-4 py-8">
				...
				<div className="border border-gray-400 rounded-lg p-6 mt-8">
					...
					<ul className="text-gray-600 space-y-1 text-sm">
						<li>
							• PACKETS ARE CREATED WITH A TYPE (MESSAGE/REACTION) AND PAYLOAD
						</li>
						<li>• PAYLOAD IS COMPRESSED USING LZ4 FRAME COMPRESSION</li>
						<li>
							• SERIALIZATION CONVERTS THE PACKET TO BINARY FORMAT USING BINCODE
						</li>
						<li>• BINARY DATA IS DISPLAYED AS HEXADECIMAL FOR READABILITY</li>
						<li>
							• DESERIALIZATION RECONSTRUCTS THE ORIGINAL PACKET FROM BINARY
							DATA
						</li>
					...
					<div className="space-y-4">
						<div>
							...
							<select
								value={selectedKind}
								onChange={(e) =>
									setSelectedKind(Number(e.target.value) as PacketKind)
								}
