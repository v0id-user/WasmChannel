...
export function useChatState() {
	...
	useEffect(() => {
		setMessages(currentMessages => {
			...
			if (deduplicatedMessages.length !== currentMessages.length) {
				console.log(`Aggressive dedup: Removed ${currentMessages.length - deduplicatedMessages.length} duplicate messages`);
	...
	const loadMoreMessages = useCallback(async () => {
		...
		if (!hasMoreMessages || !messages.length) {
			console.log("loadMoreMessages: Skipping load - hasMoreMessages:", hasMoreMessages, "messages.length:", messages.length);
			return;
		}
		...
		try {
			...
			if (response.messages && response.messages.length > 0) {
				...
				console.log("loadMoreMessages: Got", orpcMessages.length, "new messages");
				
				// Ultra-aggressive deduplication
				const currentMessageIds = new Set(messages.map(msg => msg.id));
				const currentReferenceIds = new Set(messages.map(msg => msg.refrenceId));
				...
				const newChatMessages = orpcMessages
					.filter(message => {
						...
						const isDuplicateByRef = currentReferenceIds.has(message.refrenceId);
						
						if (isDuplicateById || isDuplicateByRef) {
							console.log("loadMoreMessages: Filtering duplicate - ID:", isDuplicateById, "Ref:", isDuplicateByRef, "MessageID:", message.id);
							return false;
						}
						return true;
					})
				...
				console.log("loadMoreMessages: Filtered to", newChatMessages.length, "unique messages");
				...
				if (orpcMessages.length < 20) {
					setHasMoreMessages(false);
					console.log("loadMoreMessages: No more messages available ORPC IS NOT MEETING THE LIMIT");
				}
			} else {
		...
		if(messagesResponse) {
			...
			Array.from(uniqueMessageMap.values()).forEach(message => {
				if (!uniqueReferenceMap.has(message.refrenceId)) {
					...
					console.log("useChatState: Filtering out duplicate by reference ID:", message.refrenceId);
				}
			});
			
			const finalUniqueMessages = Array.from(uniqueReferenceMap.values());
			console.log("useChatState: After deduplication:", finalUniqueMessages.length, "unique messages");
			...
			console.log("useChatState: Setting", chatMessages.length, "unique processed messages");
