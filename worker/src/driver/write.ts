...
export class DatabaseDriver extends StorageDriver {
    ...
    async read(): Promise<WasmPacket[]> {
        ...
        return msgs.map((msg) => {
            ...
            const reactionKind = msg.reactionKind ? (parseInt(msg.reactionKind) as ReactionKind) : null;
    ...
    async getMessages(limit: number = 50, cursor?: string): Promise<DatabaseMessage[]> {
...
export class CacheDriver extends StorageDriver {
    ...
    async read(): Promise<WasmPacket[]> {
        ...
        return cachedRecords.map((record) => {
            ...
            const reactionKind = record.reactionKind ? (parseInt(record.reactionKind) as ReactionKind) : null;
    ...
    async getMessages(limit: number = 50): Promise<DatabaseMessage[]> {
        ...
        return cachedRecords.slice(0, Math.min(limit, cachedRecords.length)).map((record, index) => ({
    ...
    private async getCachedRecords(): Promise<Array<Omit<DatabaseMessage, 'id' | 'createdAt' | 'updatedAt' | 'deletedAt'>> | null> {
        const kv = this.driver as KVNamespace;
