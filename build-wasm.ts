...
const logTimeline = (step: number, title: string, message = '', state: LogState = 'active', isLast = false) => {
...
const logSubStep = (message: string, isLast = false) => {
  ...
  console.log(`${subConnector}${colors.gray}├─${colors.reset} ${colors.dim}${message}${colors.reset}`);
};

const updateTimelineStep = (step: number, state: LogState, title: string, message = '', isLast = false) => {
...
const logHeader = (title: string) => {
  ...
  console.log(`${colors.cyan}│${colors.reset} ${colors.bright}${colors.white}${title}${colors.reset} ${colors.cyan}│${colors.reset}`);
...
if (mode === 'release') {
  ...
  console.log(`${connector.last}${timelineStates.pending} ${colors.dim}Optimization skipped in dev mode${colors.reset}`);
}
...
// Step 1: Compilation
updateTimelineStep(1, 'active', '🔧 Compiling Rust to WebAssembly', `Mode: ${mode} | Target: web`);
...
try {
  ...
  const result = await $`wasm-pack ${buildArgs}`.cwd(rustWasmPath).nothrow().quiet();
  
  if (result.exitCode !== 0) {
    const errorMessage = result.stderr.toString() || `Process exited with code ${result.exitCode}`;
    updateTimelineStep(1, 'error', '🔧 Compilation Failed', errorMessage, mode !== 'release');
    logError(`Build failed: ${errorMessage}`);
    process.exit(1);
  }

  const buildTime = ((Date.now() - startTime) / 1000).toFixed(2);
  updateTimelineStep(1, 'success', '🔧 Compilation Complete', `✨ Completed in ${buildTime}s`, mode !== 'release');

  if (mode === 'release') {
    // Step 2: Optimization
    console.log(`${colors.gray}│${colors.reset}`);
    updateTimelineStep(2, 'active', '⚡ Optimizing WASM Binary', 'Running wasm-opt with size optimization');
    ...
    try {
      ...
      const optimizeResult = await $`wasm-opt -Oz --strip-target-features --strip-eh --strip-dwarf --strip-debug --strip-producers --remove-unused-module-elements --flatten --dce --vacuum --reorder-functions --reorder-locals --merge-blocks --local-cse --simplify-locals --coalesce-locals --optimize-instructions --fast-math ${wasmFile} -o ${wasmFile}`.cwd(rustWasmPath).nothrow().quiet();
      
      if (optimizeResult.exitCode !== 0) {
        const optErrorMessage = optimizeResult.stderr.toString() || `Optimization failed with code ${optimizeResult.exitCode}`;
        updateTimelineStep(2, 'error', '⚡ Optimization Failed', optErrorMessage, true);
      ...
      updateTimelineStep(2, 'success', '⚡ Optimization Complete', `✨ Completed in ${optimizeTime}s | Size: ${initialSizeKB}KB → ${finalSizeKB}KB (${reduction}% reduction)`, true);
      logSuccess(`🚀 Release build completed in ${totalTime}s`);
    } catch (optError) {
      updateTimelineStep(2, 'error', '⚡ Optimization Failed', String(optError), true);
  ...
  updateTimelineStep(1, 'error', '🔧 Compilation Failed', String(error), mode !== 'release');
